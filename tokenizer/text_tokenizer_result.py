...
class IUPAC_Tokenizer():
    def iupac_tokenizer(self, iupac):
        ...
        regex = re.compile(
            pattern + heterocycles + carbon_trivial + type_ + element + prefix_isotope + other_ions + alcane + pristavka_digit + pristavka_name + prefix_alcane + \
...
if __name__ == '__main__':
    ...
    iupac = "4-methoxy-N,N-dimethyl-benzamide"
    smiles="O1CC[C@@H](NC(=O)[C@@H](Cc2cc3cc(ccc3nc2N)-c2ccccc2C)C)CC1(C)C"
    smiles_iupac=iupac+" "+smiles
    tmp = IUPAC_Tokenizer()
    # tokens1 = tokenizer1.tokenize(text)
    print(tokenizer2.tokenize(iupac))
    print(tmp.iupac_tokenizer(iupac))
    print(tmp.iupac_tokenizer(smiles))
    print(tmp.iupac_tokenizer(smiles_iupac))
    # print(tokens1)
    # print(tokens2)